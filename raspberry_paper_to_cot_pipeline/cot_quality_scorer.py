...
class CoTQualityScorer:
    ...
    def build_criteria_columns(self, required_only: bool = False) -> List[str]:
        ...
        :return: List of column names prefixed with 'cot_quality_assessment_criteria_'
        """
        criteria = (
            constants.REQUIRED_COT_QUALITY_ASSESSMENT_CRITERIA
            if required_only
            else constants.COT_QUALITY_ASSESSMENT_CRITERIA
        )
        return [f"cot_quality_assessment_criteria_{c}" for c in criteria]
    ...
    def fetch_papers_for_scoring(self) -> Generator[sqlite3.Row, None, None]:
        ...
        return self.utils.fetch_papers_by_processing_status(
            status=constants.STATUS_COT_QUALITY_ASSESSED,
    ...
    def process_paper(self, paper: sqlite3.Row) -> None:
        """
        Execute the main logic of the COT quality scoring process.
        ...
        try:
            ...
            data = {
                "processing_status": constants.STATUS_COT_QUALITY_SCORED,
                "cot_quality_assessment_suitability_score": suitability_score,
            }
            self.utils.update_paper(paper["id"], data)
            self.logger.info(
                f"Paper {paper['paper_id']} scored and updated to status {constants.STATUS_COT_QUALITY_SCORED}, "
    ...
    def run(self) -> None:
        """Run the CoT quality scoring process."""
        self.logger.info(
            f"Starting CoT quality scoring process. Database: {self.database}, Limit: {self.limit}"
        )
        try:
            ...
            self.logger.info(
                f"CoT quality scoring process completed. Total papers scored: {processed_count}"
            )
        except Exception as e:
            self.logger.error(
                f"An error occurred during the CoT quality scoring process: {e}"
            )
            sys.exit(1)
