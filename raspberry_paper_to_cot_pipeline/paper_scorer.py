...
class PaperScorer:
    ...
    def build_criteria_columns(self) -> List:
        return [f"criteria_{c}" for c in constants.PROFILING_CRITERIA]

    def build_required_criteria_columns(self) -> List:
...
def main():
    ...
    cleaner = PaperScorer(database=args.database, limit=args.limit, debug=args.debug)
    cleaner.run()
